...
from diffusers.utils import deprecate, BaseOutput, is_torch_version, logging, USE_PEFT_BACKEND, scale_lora_layers, unscale_lora_layers
...
def UNet2DConditionModelForward(
    ...
    if cross_attention_kwargs is not None:
        cross_attention_kwargs['timestep']=int(timestep)
    else:
        cross_attention_kwargs={'timestep':int(timestep)}
    ####################################################################################################
    ...
    def forward_hook(module, input, output):
        if hasattr(module.processor, "attn_map"):
            ...
            if name not in attn_maps:
                attn_maps[name] = {}
                attn_maps[name]['average']=torch.zeros_like(module.processor.attn_map)
            else:
                attn_maps[name]['average'] = attn_maps[name]['average'] + module.processor.attn_map
                attn_maps[name][f'{module.processor.timestep}'] = module.processor.attn_map
...
def preprocess(max_height=256, max_width=256):
    temp_attn_maps={}
    for name,value in tqdm(attn_maps.items()):
        for key, attn in value.items():
            attn = torch.mean(attn.cpu(),axis=0).squeeze(0)
            _, h, w = attn.shape
            max_height = max(h, max_height)
            max_width = max(w, max_width)
            attn = F.interpolate(
                attn.to(dtype=torch.float32).unsqueeze(0),
                size=(max_height, max_width),
                mode='bilinear',
                align_corners=False
            ).squeeze(0) # (77,64,64)
            if key not in temp_attn_maps:
                temp_attn_maps[key] = {}
            temp_attn_maps[key][name] = attn
    
    attn_map={}
    for step_or_avg, attn_per_layers in temp_attn_maps.items():
        attn_map[step_or_avg] = torch.stack(list(attn_per_layers.values()), axis=0)
        attn_map[step_or_avg] = torch.mean(attn_map[step_or_avg], axis=0)
...
def visualize_and_save_attn_map(attn_map, tokenizer, prompt,folder_name='attn_maps'):
    # match with tokens
    tokens = prompt2tokens(tokenizer, prompt)
    bos_token = tokenizer.bos_token
    eos_token = tokenizer.eos_token
    pad_token = tokenizer.pad_token
    save_path = os.path.join(folder_name,"average")
    ...
    for step_or_avg, attn in attn_map.items():
        for i, (token, token_attn_map) in enumerate(zip(tokens, attn)):
            if token == bos_token:
                continue
            if token == eos_token:
                break
            token = token.replace('</w>','')
            img_name = f'{i}_<{token}>_{step_or_avg}.jpg'
            ...
            normalized_token_attn_map = cv2.applyColorMap(normalized_token_attn_map, cv2.COLORMAP_VIRIDIS)
            image = Image.fromarray(cv2.cvtColor(normalized_token_attn_map, cv2.COLOR_BGR2RGB))
            image_save_path = os.path.join(save_path, img_name)
            image.save(image_save_path)
            
            if step_or_avg != 'average':
                key = f'{i}_<{token}>'
                if key not in imgpath_per_token:
                    imgpath_per_token[key] = []
                imgpath_per_token[key].append(image_save_path)
    return imgpath_per_token
