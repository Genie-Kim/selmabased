...
data_root_path = "datasets/ShareGPT4V/data"
...
# get the full path of the experiment results
exp_result_paths = [
    os.path.join(exp_result_root, exp_result) for exp_result in exp_results
]
...
def get_result_row_list(idx):
    ...
    for result_path in results_path_list:
        if result_path is not None:
            result_imgs.append(wandb.Image(result_path))
        else:
            result_imgs.append(wandb.Image(no_image_path))

    # plus data for all columns
    row_list = (
        [idx, folder, filename, gpt4vcaption, clip_removed_text, longclip_removed_text, origin_img]
        + result_imgs
        + [question_list_str]
    )
...
one_exp_path = os.path.join(exp_result_root, exp_results[0])
for filepath in tqdm(os.listdir(one_exp_path)):
    idx = os.path.splitext(filepath)[0]
    row_list = get_result_row_list(idx)
