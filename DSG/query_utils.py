...
def generate_with_in_context_examples(
	...
	preamble: a task description for language model, starting point of prompt.
	input_name: a verbalizer for input
	output_name: a verbalizer for output
	...
	# 1) Create list of LM inputs : Create prompts using pre-built examples
	total_kwargs = []
...
def generate_dsg(id2prompts: Dict[str, Dict[str, str]],
	...
	train_examples = question_train_examples # input prompt + tuple 정보를 토대로 question을 생성
	...
	id2question_outputs = generate_with_in_context_examples(
		generate_fn=generate_fn,
		id2inputs=id2inputs,
		train_examples=train_examples, # change train_examples for question generation
		preamble=preamble, # change prompt for question generation
		num_workers=N_parallel_workers,
		verbose=verbose)
	...
	train_examples = dependency_train_examples # input prompt + tuple 정보를 토대로 dependency를 생성
	...
	id2dependency_outputs = generate_with_in_context_examples(
		generate_fn=generate_fn,
		id2inputs=id2inputs,
		train_examples=train_examples, # change train_examples for dependency generation
		preamble=preamble, # change prompt for dependency generation
		num_workers=N_parallel_workers,
		verbose=verbose)
