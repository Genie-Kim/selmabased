...
class RegionalDiffusionXLPipeline(
    ...
    def encode_prompt(
        ...
        # )
        tokenizers = [self.tokenizer, self.tokenizer] if self.tokenizer is not None else [self.tokenizer_2]
        text_encoders = (
            [self.text_encoder, self.text_encoder] if self.text_encoder is not None else [self.text_encoder_2]
        )
        ...
        pooled_prompt_embeds = pooled_prompt_embeds.repeat(1, 1, num_images_per_prompt).view(
            bs_embed * num_images_per_prompt, -1
        )
        if do_classifier_free_guidance:
            negative_pooled_prompt_embeds = negative_pooled_prompt_embeds.repeat(1, 1,num_images_per_prompt).view(
                bs_embed * num_images_per_prompt, -1
            )
