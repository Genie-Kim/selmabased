...
class RegionalDiffusionPipeline(
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if self.base_ratio==1:
                # split ratio must be 1
                ...
                for text_input_ids_elem, attention_mask_elem in zip(divided_text_input_ids,divided_attention_mask):
                    ...
                    if prompt_embeds is None:
                        ...
                        prompt_embeds = torch.cat((prompt_embeds, prompt_embeds_elem), dim=2)
            else:
                regional_prompt_list = prompt.split('BREAK')
                regional_prompt_embeds = []
                for subprompt in regional_prompt_list:
                    text_inputs = self.tokenizer(
                        subprompt,
                        padding="max_length",
                        max_length=self.tokenizer.model_max_length,
                    ...
                    text_input_ids = text_inputs.input_ids
                    untruncated_ids = self.tokenizer(subprompt, padding="longest", return_tensors="pt").input_ids
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            regional_negative_prompt = []
            for i in range(seq_len//TOKENSCON):
                ...
                regional_negative_prompt.append(negative_prompt_embeds)
            negative_prompt_embeds = torch.concat(regional_negative_prompt,dim=1)
    ...
    def check_inputs(
        ...
        # TODO : REMOVE
        # if split_ratio != "1" and self.repeat_textencoding:
        #     raise ValueError(f"Split ratio is {split_ratio} but, repeat text encoding is {self.repeat_textencoding}.")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if base_prompt is None:
            ...
            self.prompt = base_prompt + ' BREAK ' + prompt
